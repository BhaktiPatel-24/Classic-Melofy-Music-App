...
class Search: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    func fetchSongs(query: String) {
        ...
        let url = "https://itunes.apple.com/search?term=\(formattedQuery)&entity=song"
        
        AF.request(url).responseDecodable(of: SongsResponse.self) { response in
            switch response.result {
            case .success(let data):
                self.songss = data.results
                self.tableView.reloadData()
    ...
    @objc func songDidFinishPlaying(_ notification: Notification) {
        if currentSongIndex < songss.count - 1 {
            currentSongIndex += 1
        } else {
            currentSongIndex = 0
        }
